...
int save_image(t_data *data)
{
	...
	//*****HEADER************//
	int bytes_per_pixel = data->img.bpp / 8;
	char *sign = "BM";
	int height;
	int width;

	height = data->desc->y_res;
	width = data->desc->y_res;
	write(fd, sign, 2);
	...
	int resolution_x = 11811; //300 dpi
	int resolution_y = 11811; //300 dpi
	...
	for ( i = 0; i < height; i++)
	{
		...
		write(fd, &data->img.addr[pixel_offset], 1);
	}
	close(fd);
